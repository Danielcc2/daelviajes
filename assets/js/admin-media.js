...
async function requerirAdmin(){
  ...
  if (!session){ location.href = getSiteBase() + 'index.html'; throw new Error('no session'); }
  const { data, error } = await supabase.from('profiles').select('is_admin').eq('id', session.user.id).single();
  if (error || !data?.is_admin){ alert('Solo administradores.'); location.href = getSiteBase() + 'index.html'; throw new Error('no admin'); }
}
